...
public class GameView extends SurfaceView implements SurfaceHolder.Callback, Runnable {
    ...
    private final SettingsRenderer settingsR;
    ...
    public GameView(Context context){
        super(context);ctx=context;
        getHolder().addCallback(this);setFocusable(true);
        gs=new GameState();pf=new PaintFactory();
        settings=new Settings(context);gs.settings=settings;
        ps=new ParticleSystem(gs,pf);player=new Player(gs,pf);
        em=new EntityManager(gs,pf,ps);renderer=new Renderer(gs,pf);
        settingsR=new SettingsRenderer(gs,settings);
    ...
    @Override public void surfaceCreated(SurfaceHolder holder){
        gs.initScreen(getWidth(),getHeight());renderer.initStars();
    ...
    @Override public void run(){
        ...
        while(isRunning){long s=System.nanoTime();gs.animTime++;
            if(gs.gameState!=GameState.STATE_PAUSED&&gs.gameState!=GameState.STATE_SETTINGS)update();
            render();long sl=(target-(System.nanoTime()-s))/1000000;
            if(sl>0)try{Thread.sleep(sl);}catch(InterruptedException e){}}
    }

    private void update(){
        gs.vortexAngle+=0.5f;gs.shakeMag*=0.92f;
        java.util.Random rand=new java.util.Random();
        if(gs.shakeMag>0.5f){gs.shakeX=(rand.nextFloat()-0.5f)*2*gs.shakeMag;
            gs.shakeY=(rand.nextFloat()-0.5f)*2*gs.shakeMag;}
        else{gs.shakeX=0;gs.shakeY=0;}
        ps.updateParticles();ps.updatePopups();
        ...
        if(gs.gameState==GameState.STATE_MENU){
            gs.playerAngle+=1.0f;if(gs.playerAngle>=360)gs.playerAngle-=360;return;}
        if(gs.gameState==GameState.STATE_GAME_OVER){
            gs.deathFlash=Math.max(0,gs.deathFlash-0.02f);return;}

        gs.playTime++;float tm=gs.slowmoActive?0.4f:1.0f;
        player.update(tm);gs.bgHue=(gs.score/10f)%360;
        int nl=gs.score/500+1;
        if(nl>gs.level){gs.level=nl;gs.levelPopTimer=120;gs.shakeMag=10f;
            if(gs.vibEnabled())try{vibrator.vibrate(50);}catch(Exception e){}}
        if(gs.levelPopTimer>0)gs.levelPopTimer--;

        boolean wasFever=gs.feverMode;gs.feverMode=gs.combo>=5;
        if(gs.feverMode&&!wasFever){gs.feverFlash=1f;gs.shakeMag=12f;
            if(gs.vibEnabled())try{vibrator.vibrate(80);}catch(Exception e){}
        ...
        if(gs.trailOn())ps.updateTrail(px,py,tc);
        ...
        gs.obstacleTimer++;
        if(gs.obstacleTimer>=Math.max(15,(85-gs.playTime/25f)/sm)){gs.obstacleTimer=0;em.spawnObstacle();}
        ...
        em.updateOrbs(tm,feverMult);em.updateDiamonds(tm,feverMult);em.updatePowerups(tm);
    ...
    private void render(){
        ...
        try{c=h.lockCanvas();if(c==null)return;
            if(gs.gameState==GameState.STATE_SETTINGS){
                settingsR.draw(c,gs.animTime);
            }else{
                ...
                if(gs.trailOn())ps.drawTrail(c);
                if(gs.gameState!=GameState.STATE_GAME_OVER)player.draw(c);
                ps.drawParticles(c);ps.drawPopups(c);c.restore();
    ...
    @Override public boolean onTouchEvent(MotionEvent event){
        if(event.getAction()==MotionEvent.ACTION_DOWN){
            ...
            if(gs.gameState==GameState.STATE_SETTINGS){
                int r=settingsR.handleTouch(tx,ty);
                if(r==-1)gs.gameState=GameState.STATE_MENU;
            }else if(gs.gameState==GameState.STATE_MENU){
                float[] sb=renderer.getSettingsBtnBounds();
                if(tx>=sb[0]&&tx<=sb[2]&&ty>=sb[1]&&ty<=sb[3])
                    gs.gameState=GameState.STATE_SETTINGS;
                else gs.reset();
            }else if(gs.gameState==GameState.STATE_PLAYING){
                if(tx>gs.screenW-150&&ty<150){gs.gameState=GameState.STATE_PAUSED;}
                else{long diff=now-lastTapTime;
    ...
    private void gameOver(float px,float py){
        gs.gameState=GameState.STATE_GAME_OVER;gs.deathFlash=1f;gs.shakeMag=30f;
        gs.survivalTime=gs.playTime/60f;
        ...
        if(gs.score>gs.highScore){gs.highScore=gs.score;gs.isNewBest=true;
            ctx.getSharedPreferences("neonvortex",0).edit().putInt("highscore",gs.highScore).apply();}
        else gs.isNewBest=false;
        if(gs.vibEnabled())try{vibrator.vibrate(200);}catch(Exception e){}
    }
